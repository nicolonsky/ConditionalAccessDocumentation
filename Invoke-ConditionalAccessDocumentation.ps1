...
.SYNOPSIS This script retrieves all Conditional Access Policies and translates Azure AD Object ID's to display names for users, groups, directory roles, locations...
.EXAMPLE
    & "C:\Repos\GitHub\ConditionalAccessDocumentation\Invoke-ConditionalAccessDocumentation.ps1"
...
foreach ($conditionalAccessPolicy in $conditionalAccessPolicies){
    ...
    try{
        ...
        $includeUsers = @() 
        $conditionalAccessPolicy.Conditions.UserIncludeUsers | ForEach-Object {
            if (Test-Guid $PSItem){
        ...
        $excludeUsers = @()
        $conditionalAccessPolicy.Conditions.UserExcludeUsers | ForEach-Object {
            if (Test-Guid $PSItem){
                $excludeUsers += Get-MgUser -userId $PSItem | Select-Object -ExpandProperty DisplayName -ErrorAction Stop
            }else{
                $excludeUsers += $PSItem
            }
        }
        ...
        $includeGroups = @() 
        $conditionalAccessPolicy.Conditions.UserIncludeGroups | ForEach-Object {
            $includeGroups += Get-MgGroup -groupId $PSItem | Select-Object -ExpandProperty DisplayName
        }
        # Resolve object ID's of excluded groups
        $excludeGroups = @() 
        $conditionalAccessPolicy.Conditions.UserExcludeGroups | ForEach-Object {
            $excludeGroups += Get-MgGroup -groupId $PSItem | Select-Object -ExpandProperty DisplayName
        }
        # Resolve object ID's of included roles
        $includeRoles = @()
        $conditionalAccessPolicy.Conditions.UserIncludeRoles | ForEach-Object {
            $roleId = $PSItem
        ...
        $excludeRoles = @()
        $conditionalAccessPolicy.Conditions.UserExcludeRoles | ForEach-Object {
            $roleId = $PSItem
        ...
        $includeApps = @()
        $conditionalAccessPolicy.Conditions.ApplicationIncludeApplications | ForEach-Object {
        ...
        $excludeApps = @()
        $conditionalAccessPolicy.Conditions.ApplicationExcludeApplications | ForEach-Object {
            $servicePrincipalId = $PSItem
            if (Test-Guid $PSItem){
                $res = $servicePrincipals | Where-Object {$PSItem.AppId -eq $servicePrincipalId} | Select-Object -ExpandProperty DisplayName
                if ($null -ne $res){
